...
static void test_parse_array() {
    ...
    EXPECT_EQ_INT(0, (int)lept_get_array_size(&v));
    lept_free(&v);
    ...
    EXPECT_EQ_INT(LEPT_ARRAY, lept_get_type(&v));
    EXPECT_EQ_INT(4, (int)lept_get_array_size(&v));
    for (i = 0; i < 4; i++) {
        lept_value* a = lept_get_array_element(&v, i);
        EXPECT_EQ_INT(LEPT_ARRAY, lept_get_type(a));
        EXPECT_EQ_INT((int)i, (int)lept_get_array_size(a));
        for (j = 0; j < i; j++) {
...
static void test_parse_object() {
    ...
    EXPECT_EQ_INT(0, (int)lept_get_object_size(&v));
    lept_free(&v);
    ...
    EXPECT_EQ_INT(LEPT_OBJECT, lept_get_type(&v));
    EXPECT_EQ_INT(7, (int)lept_get_object_size(&v));
    ...
    EXPECT_EQ_INT(3, (int)lept_get_array_size(lept_get_object_value(&v, 5)));
    for (i = 0; i < 3; i++) {
    ...
    EXPECT_EQ_STRING("o", lept_get_object_key(&v, 6), lept_get_object_key_length(&v, 6));
    {
        ...
        for (i = 0; i < 3; i++) {
            ...
            EXPECT_EQ_INT(1, (int)lept_get_object_key_length(o, i));
...
static void test_stringify_string() {
    ...
    /* unicode不需处理，因为字符串本身是UTF-8，输出的JSON也是。只有 < 32 的才必须转义。 */
    /* TEST_ROUNDTRIP("\"\\u0024\"");                
    TEST_ROUNDTRIP("\"\\u00A2\"");                
    TEST_ROUNDTRIP("\"\\u20AC\"");                
    TEST_ROUNDTRIP("\"\\uD834\\uDD1E\"");*/
