...
static int lept_parse_object(lept_context* c, lept_value* v) {
    ...
    for (;;) {
        ...
        else  lept_parse_string_raw(c, &m.k, &m.klen);
    ...
    free(m.k);
    for (i = 0; i < size; i++)
        lept_free_object((lept_member*)lept_context_pop(c, sizeof(lept_member)));
    v->type=LEPT_NULL;
    return ret;
}
...
void lept_free_object(lept_member* m)
{
    lept_free(&m->v);
    free(m->k);
}
